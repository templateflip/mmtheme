...
class MMtheme_Posts_Widget extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array( 'classname' => 'mmtheme_posts_widget', 'description' => __( 'Display your posts with this widget', 'mmtheme' ) );
		$control_ops = array( 'id_base' => 'mmtheme_posts_widget' );
		parent::__construct( 'mmtheme_posts_widget', __( 'MMtheme Posts Widget', 'mmtheme' ), $widget_ops, $control_ops );

		$this->defaults = array(
			'title' => __( 'Featured Posts', 'mmtheme' ),
			'numposts' => 5,
			'category' => array(),
			'orderby' => 0,
			'date_limit' => 0,
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title', 'mmtheme' ); ?>:</label>
		...
		<p>
	   	 	<label for="<?php echo $this->get_field_id( 'numposts' ); ?>"><?php _e( 'Number of posts to show', 'mmtheme' ); ?>:</label>
	  ...
	  <p>
		...
		<label for="<?php echo $this->get_field_id( 'auto_detect' ); ?>"><?php _e( 'Auto detect category', 'mmtheme' ); ?></label>
		<small class="howto"><?php _e( 'If sidebar is used on single post template, display posts from current post category ', 'mmtheme' ); ?></small>
	  </p>
	...
	<p>
		<label for="<?php echo $this->get_field_id( 'date_limit' ); ?>"><?php _e( 'Only select posts which are not older than', 'mmtheme' ); ?>:</label>
	...
	<p>
	   	 <label for="<?php echo $this->get_field_id( 'manual' ); ?>"><?php _e( 'Choose manually', 'mmtheme' ); ?>:</label>
	...
	function dates_q() {
		$dates = array(
			'-1 day' => __( '1 Day', 'mmtheme' ),
			'-1 week' => __( '1 Week', 'mmtheme' ),
			'-1 month' => __( '1 Month', 'mmtheme' ),
			'-3 months' => __( '3 Months', 'mmtheme' ),
			'-6 months' => __( '6 Months', 'mmtheme' ),
			'-1 year' => __( '1 Year', 'mmtheme' ),
			'0' => __( 'Select all posts', 'mmtheme' )
		);
	...
	function widget_orderby( $widget_instance = false, $orderby = false ) {
		...
		$orders['date'] = __( 'Published date', 'mmtheme' );
		$orders['comment_count'] = __( 'Number of comments', 'mmtheme' );
		$orders['rand'] = __( 'Random', 'mmtheme' );

		if ( !empty( $widget_instance ) ) { ?>
				<label for="<?php echo $widget_instance->get_field_id( 'orderby' ); ?>"><?php _e( 'Order by:', 'mmtheme' ); ?></label>
...
?>
				<label for="<?php echo $widget_instance->get_field_id( 'category' ); ?>"><?php _e( 'Choose from:', 'mmtheme' ); ?></label><br/>
